...
VOID CScoreControl::OnWindowDestory(CD3DDevice * pD3DDevice)
{
  ...
  //销户资源
  if(m_TextureGameScore.IsNull()==false)
  {
    m_TextureGameScore.Destory();
  }
  if(m_TextureWinLoseFlag.IsNull()==false)
  {
    m_TextureWinLoseFlag.Destory();
  }
  if(m_TextureChunTian.IsNull()==false)
  {
    m_TextureChunTian.Destory();
  }
  if(m_TextureFanChunTian.IsNull()==false)
  {
    m_TextureFanChunTian.Destory();
  }
