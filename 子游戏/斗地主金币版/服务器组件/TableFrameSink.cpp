...
bool CTableFrameSink::OnEventGameConclude(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbReason)
{
  switch(cbReason)
  {
    case GER_NORMAL:    //常规结束
    {
      ...
      if((m_pGameServiceOption->wServerType&GAME_GENRE_MATCH)!=0)
      {
        for(WORD i = 0; i < GAME_PLAYER; i++)
        {
          if(i!=m_wBankerUser)
          {
            if(m_pITableFrame->GetTableUserItem(i)->IsTrusteeUser()==true)
            {
              wTrusteeCount++;
            }
          }
      ...
      for(WORD i=0; i<GAME_PLAYER; i++)
      {
        ...
        else
        {
          if(m_cbHandCardCount[m_wBankerUser]==0)
          {
            if(wTrusteeCount>0)
            {
              lUserScore=(bTrusteeUser==true)?(-2L/wTrusteeCount):0L;
            }
            else
            {
              lUserScore=-1L;
          ...
          else
          {
            if(wTrusteeCount>0)
            {
              lUserScore=(bTrusteeUser==true)?0L:(2L/wTrusteeCount);
            }
            else
            {
              lUserScore=1L;
    ...
    case GER_NETWORK_ERROR: //网络中断
    {
      ...
      //炸弹统计
      WORD lScoreTimes=1;
      for(WORD i=0; i<m_cbBombCount; i++)
      {
        lScoreTimes*=2L;
      }
...
bool CTableFrameSink::OnGameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
  ...
  switch(wSubCmdID)
  {
    case SUB_C_CALL_SCORE:  //用户叫分
    {
      ...
      if(wDataSize!=sizeof(CMD_C_CallScore))
      {
        return false;
      }
