...
void ScoreAnimation::OnRender(float offset_x, float offset_y, float hscale, float vscale) {
  ...
  if (chair_id_ == 0 || chair_id_ == 1 || chair_id_ == 2) x -= (num_count - 1) * num_width * hscale / 2;
//  else if (chair_id_ == 3) y -= (num_count - 1) * num_width * hscale / 2;
//  else if (chair_id_ == 7) y += (num_count - 1) * num_width * hscale / 2;
  else x += (num_count - 1) * num_width * hscale / 2;

  for (int i = 0; i < num_count; ++i) {
    ani_score_->SetFrame(static_cast<int>(num_temp % 10));
    if (chair_id_ <= 2) {
      ani_score_->RenderEx(x + offset_x, y + offset_y + offset_, kChairDefaultAngle[chair_id_], hscale, vscale);
      x += num_width * hscale;
    }/* else if (chair_id_ == 3) {
      ani_score_->RenderEx(x + offset_x + offset_, y + offset_y, kChairDefaultAngle[chair_id_], hscale, vscale);
      y += num_width * hscale;
    } else if (chair_id_ == 7) {
      ani_score_->RenderEx(x + offset_x + offset_, y + offset_y, kChairDefaultAngle[chair_id_], hscale, vscale);
      y -= num_width * hscale;
...
Fish::Fish(FishKind fish_kind, int fish_id, int fish_multiple, float fish_speed, float bounding_box_width, float bounding_box_height, float hit_radius)
...
bool Fish::OnFrame(float delta_time, bool lock) {
  ...
  if (fish_status_ == FISH_ALIVE) {
    ...
    if (!ani_fish_die_->IsPlaying()) {
	  ...
	  SoundManager::GetInstance().OnFrame(delta_time);
    }
  }
...
bool Fish::BulletHitTest(Bullet* bullet) {
  ...
  if (box.ComputeCollision(point.x, point.y)) {
    if (fish_kind() == FISH_KIND_21 && bullet->bullet_mulriple() == GameManager::GetInstance().max_bullet_multiple()) {
      WORD me_chair_id = GameManager::GetInstance().GetClientKernel()->GetMeChairID();
