...
current_fish_lk_multiple_(50), android_chairid_(INVALID_CHAIR) {
	...
	memset(m_pRateInc, 0, sizeof(m_pRateInc));
	m_nRateTimeCount = 0;
	m_nTimeSum = 0;
	m_nStockTime = mt_rand(30, 40);
	memset(m_pLastGunScore, 0, sizeof(m_pLastGunScore));
}
...
bool TableFrameSink::Initialization(IUnknownEx* unknownex) {
	...
	if (!_taccess(save_file, 0)) { // if save data file exist
		...
		m_lTableStock = 1000000L;
	}
	m_nRateTime = mt_rand(6, 12) * 10;
...
bool TableFrameSink::OnTimerStockOperate(WPARAM bind_param) {
	if (m_nTimeSum++ >= m_nStockTime) {
		m_nStockTime = mt_rand(30, 40);
		m_nTimeSum = 0;
		int rate = mt_rand(5, 10);
		SCORE score = m_lTableStock * rate / 100;
		m_lBonusStock += score;
		m_lTableStock -= score;
	}
	...
	if (m_nRateTimeCount++ >= m_nRateTime) {
		m_nRateTime = mt_rand(6, 12) * 10;
		memset(m_pRateInc, 0, sizeof(m_pRateInc));
		for (int i = 0; i < 3; i++) {
			m_pRateInc[mt_rand(0, 5)] = mt_rand(8, 10) * 10;
		}
	}
	return true;
}
...
bool TableFrameSink::OnSubUserFire(IServerUserItem * server_user_item, BulletKind bullet_kind, float angle, int bullet_mul, int lock_fishid) {
	...
	if (bullet_mul > m_pLastGunScore[chair_id]) {
		if (m_pRateInc[chair_id] != 0) {
			m_nRateTimeCount = 1000;
		}
	}
	m_pLastGunScore[chair_id] = bullet_mul;
...
bool TableFrameSink::OnSubCatchFish(IServerUserItem* server_user_item, int fish_id, BulletKind bullet_kind, int bullet_id, int bullet_mul) {
	...
	if (m_pRateInc[server_user_item->GetChairID()] != 0) {
		double incProbablity = (double)m_pRateInc[server_user_item->GetChairID()] / 100.0 + 1.0;
		fish_probability *= incProbablity;
	}
