...
const SCORE g_cpTableLevelsC[27] = { 1000000, 900000, 800000, 700000, 600000, 500000, 400000, 300000, 200000, 900000, 800000, 700000, 600000, 500000, 400000, 300000, 200000, 700000, 600000, 500000, 400000, 300000, 200000, 500000, 400000, 300000, 200000 };
...
bool TableFrameSink::OnTimerStockOperate(WPARAM bind_param) {
	// 彩金抽水
	if (m_nStockTime <= ++m_nTimeSum) {
		m_nTimeSum = 0;
		m_nStockTime = mt_rand(1, 5) * 60;
		SCORE lBonusRevenue = m_lTableStock * 5 / 100;
		m_lTableStock -= lBonusRevenue;
		m_lBonusStock += lBonusRevenue;
		ZeroMemory(g_lpStr, sizeof(g_lpStr));
		_stprintf(g_lpStr, TEXT("彩金：%I64d   奖池：%I64d"), m_lBonusStock, m_lTableStock);
		CTraceService::TraceString(g_lpStr, TraceLevel_Info);
	}
	// 彩金发放
	// m_lBonusRandom = g_cpBonusLevelsA[mt_rand(0, 29)]; // 发放彩金随机数
	if (m_lBonusRandom <= m_lBonusStock) {
		m_lBonusRandom = g_cpBonusLevelsA[mt_rand(0, 29)];
		m_lTableStock = m_lBonusStock;
		m_lBonusStock = 0;
		ZeroMemory(g_lpStr, sizeof(g_lpStr));
		_stprintf(g_lpStr, TEXT("彩金：%I64d   奖池：%I64d"), m_lBonusStock, m_lTableStock);
		CTraceService::TraceString(g_lpStr, TraceLevel_Info);
...
bool TableFrameSink::OnSubUserFire(IServerUserItem * server_user_item, BulletKind bullet_kind, float angle, int bullet_mul, int lock_fishid) {
	...
	if (!server_user_item->IsAndroidUser()) { // 如果玩家不是机器人
		...
		m_lTableStock += bullet_mul - revenue; // 增加座位库存并抽水
...
bool TableFrameSink::OnSubCatchFish(IServerUserItem* server_user_item, int fish_id, BulletKind bullet_kind, int bullet_id, int bullet_mul) {
	...
	if (!server_user_item->IsAndroidUser())
	{
		if(m_lTableStock < 0) return true;
	}
	else
	{
...
#ifndef TEST
	if (!server_user_item->IsAndroidUser() && m_lTableStock - less_score < 0) return true;
	...
	if (!server_user_item->IsAndroidUser())
	{
		if(fish_trace_info->fish_kind == FISH_KIND_23 && m_lTableStock < bomb_stock_)
			fish_probability = 0;
		if (fish_trace_info->fish_kind == FISH_KIND_24 && m_lTableStock < super_bomb_stock_)
			fish_probability = 0;
	}
	else
	{
	...
	{
		if (m_lTableStock >= stock_crucial_score_[stock_crucial_count])
		{
