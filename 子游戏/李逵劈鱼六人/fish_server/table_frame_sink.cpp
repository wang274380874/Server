...
// 	TCHAR szOut[128];
// 	sprintf_yy(szOut, _T("game_service_option_->wServerID = %d"), game_service_option_->wServerID);
...
bool TableFrameSink::OnEventGameConclude(WORD chair_id, IServerUserItem* server_user_item, BYTE reason)
{
	if (reason == GER_DISMISS)
	{
		for (WORD i = 0; i < GAME_PLAYER; ++i)
		{
			...
			//edit by yang yang  2015年7月21日  星期二  14:23:57
		//	OutputDebugString(_T("捕鱼 TableFrameSink::OnEventGameConclude reason == GER_DISMISS"));
			//end
	...
	{
		//edit by yang yang  2015年7月21日  星期二  14:24:14
	//	OutputDebugString(_T("chair_id < GAME_PLAYER && server_user_item != NULL"));
		//end
...
bool TableFrameSink::OnEventSendGameScene(WORD chair_id, IServerUserItem* server_user_item, BYTE game_status, bool send_secret)
{
	switch (game_status)
	{
		...
		case GAME_STATUS_PLAY:
			...
			//edit by yang yang 二〇一八年七月二十五日 16:12:35
			memset(&gamestatus, 0, sizeof(CMD_S_GameStatus));
			gamestatus.cbBackIndex = 0;
			gamestatus.lPlayScore = 0;
			for (int nIndex = 0;nIndex < GAME_PLAYER; nIndex++)
			{
				gamestatus.lPlayCurScore[nIndex] = 2;
			}
			for (int nIndex = 0;nIndex < GAME_PLAYER; nIndex++)
			{
				gamestatus.lPlayStartScore[nIndex] = 2;
			}
			for (int nIndex = 0;nIndex < 26; nIndex++)
			{
				for (int j = 0;j < 2;j++)
				{
					gamestatus.nFishMultiple[nIndex][j] = 4;
				}
			...
			for (int nIndex = 0;nIndex < 26; nIndex++)
			{
				for (int j = 0;j < 6;j++)
				{
					gamestatus.lPlayFishCount[nIndex][j] = 5;
				}
				
			}
			gamestatus.lCellScore = 10;
			gamestatus.nBulletVelocity = 10;
			gamestatus.nBulletCoolingTime = 10;
			gamestatus.nMaxTipsCount = 10;
			for (int nIndex = 0;nIndex < GAME_PLAYER; nIndex++)
			{
				gamestatus.lBulletConsume[nIndex] = 2;
			}
			for (int nIndex = 0;nIndex < GAME_PLAYER; nIndex++)
			{
				gamestatus.nMultipleValue[nIndex] = 2;
			}
			for (int nIndex = 0;nIndex < GAME_PLAYER; nIndex++)
			{
				gamestatus.nMultipleIndex[nIndex] = 2;
			}
			gamestatus.bUnlimitedRebound = false;
...
bool TableFrameSink::OnGameMessage(WORD sub_cmdid, void* data, WORD data_size, IServerUserItem* server_user_item)
{
	switch (sub_cmdid)
	{
		...
		{
// 								TCHAR szOut[128];
// 								sprintf_yy(szOut, _T("  SUB_C_USER_FIRE  data_size = %d,sizeof(CMD_C_UserFire) = %d"), data_size, sizeof(CMD_C_UserFire));
// 						//		OutputDebugString(szOut);
...
bool TableFrameSink::LoadConfig()
{
	...
	{
		OutputDebugString(_T("xml_doc.LoadFile"));
		return false;
	}
	...
	if (xml_element == NULL)
	{
		OutputDebugString(_T("xml_element == NULL"));
		return false;
	}
...
// 				TCHAR szOut[256];
// 				sprintf_yy(szOut, _T("stock_crucial_count_ = %d stock_crucial_score_[stock_crucial_count_] = %ld,stock_increase_probability_[stock_crucial_count_] = %ld"), stock_crucial_count_, stock_crucial_score_[stock_crucial_count_], stock_increase_probability_[stock_crucial_count_]);
// 			//	OutputDebugString(szOut);
		...
		else if (!strcmp(xml_child->Value(), "Fish"))
		{
			...
			{
				OutputDebugString(_T("if (fish_kind >= FISH_KIND_COUNT || fish_kind < 0)"));
				return false;
			}
		...
		else if (!strcmp(xml_child->Value(), "Bullet"))
		{
			...
			{
				OutputDebugString(_T("if (bullet_kind >= BULLET_KIND_COUNT || bullet_kind < 0)"));
				return false;
			}
	...
	{
		OutputDebugString(_T("if (fish_count != FISH_KIND_COUNT)"));
		return false;
	}
	if (bullet_kind_count != BULLET_KIND_COUNT)
	{
		OutputDebugString(_T("if (bullet_kind_count != BULLET_KIND_COUNT)"));
		return false;
	}
...
bool TableFrameSink::OnSubUserFire(IServerUserItem * server_user_item, BulletKind bullet_kind, float angle, int bullet_mul, int lock_fishid)
{
	...
	if (fish_score_[chair_id] < bullet_mul)  //用户分数 不够 发炮
	{
		if (server_user_item->IsAndroidUser())
		{
			table_frame_->PerformStandUpAction(server_user_item);
...
//	TCHAR szOut[128];
//	sprintf_yy(szOut, _T("  OnSubUserFire  bullet_info->bullet_kind = %d,bullet_info->bullet_mulriple = %d"), 
// 		bullet_info->bullet_kind,
// 		bullet_info->bullet_mulriple);
//	OutputDebugString(szOut);
	return true;
}
...
bool TableFrameSink::OnSubCatchFish(IServerUserItem* server_user_item, int fish_id, BulletKind bullet_kind, int bullet_id, int bullet_mul)
{
	...
	if (!server_user_item->IsAndroidUser())
	{
		if (g_stock_score_ < 0) return true;
	}
	else
	{
	...
	{
		if (bullet_energy[server_user_item->GetChairID()] == -1)
		{
// 			TCHAR szOut[256];
// 			sprintf_yy(szOut, _T("bullet_energy[server_user_item->GetChairID()] = %d"), bullet_energy[server_user_item->GetChairID()]);
// 		//	OutputDebugString(szOut);
			return true;
		}
		if (bullet_energy[server_user_item->GetChairID()] != 0 && bullet_id > bullet_energy[server_user_item->GetChairID()])
		{
// 			TCHAR szOut[256];
// 			sprintf_yy(szOut, _T("bullet_energy[server_user_item->GetChairID()] = %d, bullet_id = %d"), bullet_energy[server_user_item->GetChairID()], bullet_id);
// 		//	OutputDebugString(szOut);
			return true;
		}
		fish_score *= 2;
...
#ifndef TEST
	if (!server_user_item->IsAndroidUser() && g_stock_score_ - less_score < 0)
	{
// 		TCHAR szOut[256];
// 		sprintf_yy(szOut, _T("g_stock_score_ = %I64d, less_score = %I64d"), g_stock_score_, less_score);
// 	//	OutputDebugString(szOut);
		return true;
	}
	...
	{
	//	TCHAR szOut[256];
	//	sprintf_yy(szOut, _T("stock_crucial_score_[stock_crucial_count] = %d"), stock_crucial_score_[stock_crucial_count]);
	////	OutputDebugString(szOut);
	//	sprintf_yy(szOut, _T("stock_crucial_count = %d g_stock_score_ = %I64d,stock_crucial_score_[stock_crucial_count] = %d"), stock_crucial_count, g_stock_score_, stock_crucial_score_[stock_crucial_count]);
	////	OutputDebugString(szOut);
		if (g_stock_score_ >= stock_crucial_score_[stock_crucial_count])
		{
		//	OutputDebugString(_T("-----------------test-------------"));
	//		sprintf_yy(szOut, _T("probability = %f fish_probability = %f,stock_increase_probability_[stock_crucial_count] = %f"), probability, fish_probability, stock_increase_probability_[stock_crucial_count]);
	//		OutputDebugString(szOut);
...
bool TableFrameSink::OnSubCatchSweepFish(IServerUserItem* server_user_item, int fish_id, int* catch_fish_id, int catch_fish_count)
{
	...
	if (!server_user_item->IsAndroidUser())
	{
		if (g_stock_score_ - fish_score < 0)
		{
			return true;
		}
		g_stock_score_ -= fish_score;
...
bool TableFrameSink::OnSubStockOperate(IServerUserItem* server_user_item, unsigned char operate_code)
{
	...
	{
		CString str;
		str.Format(TEXT("【%s】清空了库存%I64d。"), server_user_item->GetNickName(), g_stock_score_);
...
void TableFrameSink::CalcScore(IServerUserItem* server_user_item)
{
	...
	////edit by yang yang  2015年7月21日  星期二  14:22:04
	//TCHAR szOut[512];
	//sprintf_yy(szOut, _T("捕鱼 TableFrameSink::CalcScore score_info.lScore = %I64d,fish_score_[chair_id] = %I64d，exchange_fish_score_[chair_id] = %I64d,exchange_ratio_userscore_ = %d,exchange_ratio_fishscore_ = %d"),
	//	score_info.lScore,
	//	fish_score_[chair_id],
	//	exchange_fish_score_[chair_id],
	//	exchange_ratio_userscore_,
	//	exchange_ratio_fishscore_);
	//OutputDebugString(szOut);
	////end
