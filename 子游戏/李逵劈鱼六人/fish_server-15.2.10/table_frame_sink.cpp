...
const DWORD kRevenueTimer = 27;

const DWORD kClearTraceElasped = 60;
const DWORD kBuildSmallFishTraceElasped = 2;
const DWORD kBuildMediumFishTraceElasped = 7;
const DWORD kBuildFish18TraceElasped = 21;
const DWORD kBuildFish19TraceElasped = 30;
const DWORD kBuildFish20TraceElasped = 43;
const DWORD kBuildFishLKTraceElasped = 54;
const DWORD kBuildFishBombTraceElasped = 66;
const DWORD kBuildFishSuperBombTraceElasped = 70;
const DWORD kBuildFishLockBombTraceElasped = 75;
const DWORD kBuildFishSanTraceElasped = 85;
const DWORD kBuildFishSiTraceElasped = 95;
...
const SCORE g_cpBonusLevelsB[30] = {30000000,20000000,15000000,10000000,8000000,5000000,3000000,1000000,800000,500000,20000000,15000000,10000000,8000000,5000000,30000000,1000000,800000,500000,10000000,8000000,5000000,3000000,1000000,800000,500000,3000000,1000000,800000,500000};
const SCORE g_cpTableLevelsC[12] = { 1000000, 900000, 800000, 700000, 600000, 500000, 400000, 300000, 200000, 400000, 300000, 200000 };

const short g_cpRevenues[3] = {6, 9, 12};
...
current_fish_lk_multiple_(50), android_chairid_(INVALID_CHAIR) {
	...
	m_lTableStock = 0; // 桌台库存
	memset(m_pTimeSum, 0, sizeof(m_pTimeSum)); // 发炮计数
	memset(m_pStockRandom, 0, sizeof(m_pTimeSum)); // 归档随机数
	for(int i = 0; i < GAME_PLAYER; i++) {
		m_pStockRandom[i] = mt_rand(1, 3) * 10;
	}
	m_lBonusStock = 0; // 彩金
	...
	m_lTableRandom = g_cpTableLevelsC[mt_rand(0, 11)];
	m_nDist1 = mt_rand(0, 5);
	m_nDist2 = mt_rand(0, 5);
	stock_crucial_count_ = 0;
	m_nDistTimes = mt_rand(1, 6);
}
...
TableFrameSink::~TableFrameSink() {
	...
	for (int i = 0; i < GAME_PLAYER; i++) {
		m_lTableStock += m_pChairsStock[i];
		m_pChairsStock[i] = 0;
	}
	TCHAR str[128];
	ZeroMemory(str, sizeof(str));
	_stprintf(str, _T("当前库存：%I64d。"), m_lTableStock);
	CTraceService::TraceString(str, TraceLevel_Info);

	FILE * file = _tfopen(save_file, TEXT("wb"));
	fwrite(&m_lTableStock, sizeof(LONGLONG), 1, file);
	fwrite(&m_lTableRandom, sizeof(LONGLONG), 1, file);
	fwrite(&m_lBonusStock, sizeof(LONGLONG), 1, file);
	fwrite(&m_lBonusRandom, sizeof(LONGLONG), 1, file);
	fwrite(&m_lTempBonus, sizeof(LONGLONG), 1, file);
	fwrite(&m_nCurrentPlan, sizeof(int), 1, file);
	fwrite(&m_nNextPlan, sizeof(int), 1, file);
	fwrite(&m_nDeployBonusTime, sizeof(int), 1, file);
	fwrite(&m_nDeployBonusRandom, sizeof(int), 1, file);
	fflush(file);
	fclose(file);
...
bool TableFrameSink::Initialization(IUnknownEx* unknownex) {
	...
	if (!_taccess(save_file, 0)) { // if save data file exist
		...
		FILE * file = _tfopen(save_file, TEXT("rb"));
		Marshal* marshal = new Marshal(file);
		m_lTableStock = marshal->loadLong();
		TCHAR str[128];
		ZeroMemory(str, sizeof(str));
		_stprintf(str, _T("当前库存：%I64d。"), m_lTableStock);
		CTraceService::TraceString(str, TraceLevel_Info);
		m_lTableRandom = marshal->loadLong();
		...
		delete marshal;
		fclose(file);
		SCORE lChairScore = m_lTableStock / 6;
		SCORE delta = m_lTableStock - (lChairScore * 6);
		for(int i = 0; i < GAME_PLAYER; i++) {
			m_pChairsStock[i] = lChairScore;
			m_pTimeSum[i] = 0;
			m_pStockRandom[i] = mt_rand(1, 3) * 10;
		}
		m_pChairsStock[mt_rand(0, 5)] += delta;
		m_lTableStock = 0;
	}
	else {
		m_nCurrentPlan = 0; // first time is plan A
		m_nNextPlan = 0; // second time is plan A too
		m_lBonusStock = 1000000L;
		for(int i = 0; i < GAME_PLAYER; i++) {
			m_pChairsStock[i] = 100000L;
		}
		m_lTableStock = 400000L;
	}
	m_sCurrentRevenue = g_cpRevenues[mt_rand(0, 2)];
	memset(m_pPaidForStock, 0, sizeof(m_pPaidForStock));
...
bool TableFrameSink::OnTimerMessage(DWORD timer_id, WPARAM bind_param) {
	...
	case kRevenueTimer:
		return OnTimerRevenue(bind_param);
	default:
		ASSERT(FALSE);
	}
	return false;
...
void TableFrameSink::StartAllGameTimer() {
	...
	table_frame_->SetGameTimer(kRevenueTimer, 43200000, kRepeatTimer, 0);
}
...
void TableFrameSink::KillAllGameTimer() {
	...
	table_frame_->KillGameTimer(kRevenueTimer);
}
...
bool TableFrameSink::OnTimerBuildSmallFishTrace(WPARAM bind_param) {
	...
	BuildFishTrace(3 + rand() % 6, FISH_KIND_1, FISH_KIND_10);
	return true;
}

bool TableFrameSink::OnTimerBuildMediumFishTrace(WPARAM bind_param) {
	BuildFishTrace(2 + rand() % 3, FISH_KIND_11, FISH_KIND_17);
	return true;
}
...
/*
void TableFrameSink::DistributePlanA() {
	if (--m_nDistTimes == 0) {
		m_nDist1 = mt_rand(0, 5);
		m_nDist2 = mt_rand(0, 5);
		m_nDistTimes = mt_rand(1, 4);
	}
	int total_gun = m_pGunsScore[m_nDist1] + m_pGunsScore[m_nDist2];
	int chair1Gun = (m_pGunsScore[m_nDist1] * 1000 / total_gun + 300) / 2;
	int chair2Gun = (m_pGunsScore[m_nDist2] * 1000 / total_gun + 700) / 2;
	SCORE chair1Score = m_lTableStock * chair1Gun / 1000;
	SCORE chair2Score = m_lTableStock * chair2Gun / 1000;
	SCORE dest = m_lTableStock - (chair1Score + chair2Score);
	chair2Score += dest;
	m_pChairsStock[m_nDist1] += chair1Score;
	m_pChairsStock[m_nDist2] += chair2Score;
	m_lTableStock = 0;
}

void TableFrameSink::DistributePlanB() {
	if (--m_nDistTimes == 0) {
		m_nDist1 = mt_rand(0, 5);
		m_nDist2 = mt_rand(0, 5);
		m_nDistTimes = mt_rand(1, 4);
	...
	int total_gun = m_pGunsScore[m_nDist1] + m_pGunsScore[m_nDist2];
	int chair1Gun = (m_pGunsScore[m_nDist1] * 1000 / total_gun + 200) / 2;
	int chair2Gun = (m_pGunsScore[m_nDist2] * 1000 / total_gun + 800) / 2;
	SCORE chair1Score = m_lTableStock * chair1Gun / 1000;
	SCORE chair2Score = m_lTableStock * chair2Gun / 1000;
	SCORE dest = m_lTableStock - (chair1Score + chair2Score);
	chair2Score += dest;
...
void TableFrameSink::DistributePlanC() {
	if (--m_nDistTimes == 0) {
		m_nDist1 = mt_rand(0, 5);
		m_nDist2 = mt_rand(0, 5);
		m_nDistTimes = mt_rand(1, 4);
	...
	int total_gun = m_pGunsScore[m_nDist1] + m_pGunsScore[m_nDist2];
	int chair1Gun = (m_pGunsScore[m_nDist1] * 1000 / total_gun + 400) / 2;
	int chair2Gun = (m_pGunsScore[m_nDist2] * 1000 / total_gun + 600) / 2;
	SCORE chair1Score = m_lTableStock * chair1Gun / 1000;
	SCORE chair2Score = m_lTableStock * chair2Gun / 1000;
	SCORE dest = m_lTableStock - (chair1Score + chair2Score);
	chair2Score += dest;
...
bool TableFrameSink::OnTimerStockOperate(WPARAM bind_param) {
	...
	if (m_nBonusElapsed >= m_nBonusReCalc) {
		m_nBonusReCalc = mt_rand(20, 60) * 60;
		m_nBonusElapsed = 0;
		int rates[8] = {9, 7, 5, 7, 5, 5, 0, 0};
		m_nBonusRevenue = rates[mt_rand(0, 7)];
	}
	// 判断是否触及彩金发放条件
	if (m_lBonusStock >= m_lBonusRandom) {
		...
		m_nNextPlan = (mt_rand(1, 100) > 80 ? 1 : 0);
		/*if (m_nCurrentPlan == 1) {
	...
	if (m_lTempBonus > 0) {
		m_nDeployBonusTime++;
		if (m_nDeployBonusTime >= m_nDeployBonusRandom * 60) {
			// 填满库存
			// 计算库存剩余分数
			SCORE delta = m_lTableRandom - m_lTableStock;
			if (m_lTempBonus <= delta) {
				m_lTableStock += m_lTempBonus;
				m_lTempBonus = 0;
			}
			else {
				m_lTableStock = m_lTableRandom;
				m_lTempBonus -= delta;
			}
			// 重新分配过渡库存分发时间
			int pDeployTimeLevels[20] = { 30, 30, 30, 40, 40, 40, 50, 50, 60, 70, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150 };
			m_nDeployBonusRandom = pDeployTimeLevels[mt_rand(0, 19)];
			m_nDeployBonusTime = 0;
		}
	}
	// 小库存归档
	...
	for (i = 0; i < GAME_PLAYER; i++) {
		m_pTimeSum[i] ++;
		if (m_pTimeSum[i] >= m_pStockRandom[i]) {
			// 彩金抽水
			SCORE nBonusRevenue = m_nBonusRevenue * m_pChairsStock[i] / 100;
			m_lTableStock += m_pChairsStock[i] - nBonusRevenue;
			m_lBonusStock += nBonusRevenue;
			TCHAR cstr[512];
			ZeroMemory(cstr, sizeof(cstr));
			_stprintf(cstr, TEXT("玩家 %d 的小库存于 %I64d 时进行归档，彩金抽水：%I64d，当前彩金：%I64d，当前库存：%I64d"), i, m_pChairsStock[i], nBonusRevenue, m_lBonusStock, m_lTableStock);
			CTraceService::TraceString(cstr, TraceLevel_Info);
			m_pChairsStock[i] = 0;
			m_pTimeSum[i] = 0;
			m_pStockRandom[i] = mt_rand(1, 3) * 10;
		}
	...
	// 大奖池分发
	if (m_lTableStock >= m_lTableRandom) {
		m_nDist1 = mt_rand(0, 5);
		do {
			m_nDist2 = mt_rand(0, 5);
		} while (m_nDist2 == m_nDist1);
		/*
		int rnd = mt_rand(0, 2);
		switch (rnd) {
		case 0:
		...
		// 如果两人的贡献值不同，则按贡献值分配奖金
		//double ratio = (double)m_pPaidForStock[m_nDist1] / ((double)(m_pPaidForStock[m_nDist1] + m_pPaidForStock[m_nDist2]));
		LONGLONG ratio = (m_pPaidForStock[m_nDist1] * 10000) / (m_pPaidForStock[m_nDist1] + m_pPaidForStock[m_nDist2]);
		SCORE score = 0;
		score = (ratio * m_lTableStock) / 10000;
		m_lTableStock -= score;
		m_pChairsStock[m_nDist1] += score;
		m_pChairsStock[m_nDist2] += m_lTableStock;
		m_lTableStock = 0;
		memset(m_pPaidForStock, 0, sizeof(m_pPaidForStock));
		//m_nTableStockPlan = mt_rand(0, 2);
		m_lTableRandom = g_cpTableLevelsC[mt_rand(0, 11)];
...
bool TableFrameSink::OnTimerRevenue(WPARAM bind_param) {
	m_sCurrentRevenue = g_cpRevenues[mt_rand(0, 2)];
	return true;
}
//////////////////////////////////////////////////////////////////////
...
bool TableFrameSink::OnSubUserFire(IServerUserItem * server_user_item, BulletKind bullet_kind, float angle, int bullet_mul, int lock_fishid) {
	...
	if (!server_user_item->IsAndroidUser()) { // 如果玩家不是机器人
		int revenue = (int)ceil(m_sCurrentRevenue * (double)bullet_mul / 1000.0); // 计算抽水，当炮的倍数小于等于100时抽取1分的水
		...
		////////////////////////////////////////////////////////
		m_pPaidForStock[chair_id] += bullet_mul - revenue;
		///////////////////////////////////////////////////////
	}
	else
	{
...
bool TableFrameSink::OnSubStockOperate(IServerUserItem* server_user_item, unsigned char operate_code) {
	...
	if (operate_code == 0) {
		stock_operate_result.stock_score = m_lTableStock;
	} else if (operate_code == 1) {
		CString str;
		str.Format(TEXT("【%s】清空了库存%I64d。"), server_user_item->GetNickName(),m_lTableStock);
		stock_operate_result.stock_score = m_lTableStock = 0;	
		CTraceService::TraceString(str, TraceLevel_Info);
	} else if (operate_code == 2) {
		m_lTableStock += game_service_option_->lCellScore;
		stock_operate_result.stock_score = m_lTableStock;
		CString str;
