...
const DWORD kBuildFishSanTraceTimer = 51;
const DWORD kBuildFishSiTraceTimer = 52;
const DWORD kBuildFishKingTraceTimer = 53;
const DWORD kSwitchSceneTimer = 54;
const DWORD kSceneEndTimer = 55;
const DWORD kLKScoreTimer = 56;
const DWORD kFangShuiTimer = 57;				//放水检测
const DWORD dongdongbo = 58;             //波动
const DWORD jiqirenhuanzhuang = 59;
const DWORD min5jilu = 60;
const DWORD daodiantichu = 61;
const DWORD yifenzhongbaocun = 62;
const DWORD TBuildFish41TraceTimer = 63;


const  char * fdd = nullptr;
...
//子弹最小最大值
static int g_bullet_multiple_[2] = { 1000, 9900 };
//大鱼库存档位
...
//每张桌子机器人最小个数
static int g_TableMinAndroidCount = 8;
//每张桌子机器人最大个数
static int g_TableMaxAndroidCount = 8;
...
bool CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	////780C6EF16B0B66EF284C0416B9D6D5D0uu
	//if (StrCmp(szMachineID, TEXT("37565C6D764025CCBEEC96446DF4B94C")) != 0)
	//{
		//AfxMessageBox(TEXT("非授权机器，请联系...."));u
	//	return false;
	//u}
...
//						return	m_pITableFrame->sendcomputer(&f,0);
	...
	case SUB_C_ip:
	{
					 ...
					 for (auto i : iphao1)
					 {
						 shuliang.iphao[j] = i.ip23;
						 shuliang.gameid[j++] = i.id23;
					 }
	...
	case SUB_C_ADMIN_FANGSHUI:
	{
		...
		if (wDataSize == 0)
		{
			...
			if (panduan1 && !g_bFangShuiStart)
			{
				...
				TCHAR szJiluPath[MAX_PATH + 50] = { 0 };
				_snwprintf_s(szJiluPath, CountArray(szJiluPath), TEXT("%s//GameModule/jilu1.txt"), szWorkDir);

				FILE *wenjian = _wfopen(szJiluPath, L"a+,ccs=UTF-16LE");
				if (wenjian)
				{
				...
				{
					_snwprintf_s(szJiluPath, CountArray(szJiluPath), TEXT("%s//GameModule/jilu1.txt"), szWorkDir);
					FILE *wenjian2 = _wfopen(szJiluPath, L"w,ccs=UNICODE");
...
bool CTableFrameSink::OnActionUserSitDown(WORD chair_id, IServerUserItem* server_user_item, bool lookon_user) 
{
	...
	if (!lookon_user)
	{
		...
		if (m_pITableFrame->GetGameStatus() == GAME_STATUS_FREE)
		{
			...
			m_pITableFrame->SetGameTimer(daodiantichu, 1000, kRepeatTimer, 0);
...
void CTableFrameSink::SaveSweepFish(FishKind fish_kind, int fish_id, BulletKind bullet_kind, int bullet_mulriple) {
...
bool CTableFrameSink::LoadConfig() 
{
	...
	_snwprintf_s(fileName, CountArray(fileName), TEXT("%s//GameModule/lkpy_Config_%d.xml"), szWorkDir, m_pGameServiceOption->wServerID);
	
	TCHAR szJiluPath[MAX_PATH+50] = { 0 };
	_snwprintf_s(szJiluPath, CountArray(szJiluPath),TEXT("%s//GameModule/jilu2.txt"), szWorkDir);
	
	FILE *wenjian2 = _wfopen(szJiluPath, L"a+,ccs=UTF-16LE");
	...
	{
		_snwprintf_s(szJiluPath, CountArray(szJiluPath), TEXT("%s//GameModule/jilu1.txt"), szWorkDir);
		FILE *wenjian2 = _wfopen(szJiluPath, L"w,ccs=UTF-16LE");
	...
	for (xmlChild = xmlElement->FirstChildElement(); xmlChild != NULL; xmlChild = xmlChild->NextSiblingElement())
	{
		...
		else if (!strcmp(xmlChild->Value(), "Cannon"))
		{
			...
			char* temp = NULL;
			g_bullet_multiple_[0] = strtol(attri, &temp, 10);
			g_bullet_multiple_[1] = strtol(temp + 1, &temp, 10);
		}
		//大鱼小鱼默认库存
...
bool CTableFrameSink::OnTimerjilu(WPARAM bind_param)
{
	...
	for (WORD i = 0; i < GAME_PLAYER; ++i)
	{
		...
		char * fx = inet_ntoa(a);
		strbuffer.Format(TEXT("%s"), CA2T(fx).m_psz);		
		...
		{
			_snwprintf_s(szJiluPath, CountArray(szJiluPath), TEXT("%s//GameModule/baojing.txt"), szWorkDir);
			FILE *wenjian3 = _wfopen(szJiluPath, L"a+,ccs = UTF-16LE");
...
bool CTableFrameSink::OnSubUserFire(IServerUserItem * server_user_item, int bullet_id, BulletKind bullet_kind, float angle, int bullet_mul, int lock_fishid, float fire_speed, bool bBadManCheat) {
  WORD chair_id = server_user_item->GetChairID();
...
bool CTableFrameSink::OnSubCatchFish(IServerUserItem* server_user_item, int fish_id, BulletKind bullet_kind, int bullet_id, int bullet_mul)
{
	...
	fish_probability *= paogailv[bullet_mul / g_bullet_multiple_[0]-1];
...
bool CTableFrameSink::OnSubshanchujilu(IServerUserItem* server_user_item, int zz)
{
	...
	FILE *wenjian3 = _wfopen(szJiluPath, L"w,ccs = UTF-16LE");
