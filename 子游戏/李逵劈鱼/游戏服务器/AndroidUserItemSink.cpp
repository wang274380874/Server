...
#define TIME_SWITCH_SCENE_END       20									// 场景机器人瞄准时间
#define TIME_SWITCH_SCENE_ANDROID_STOP     27									// 场景机器人停止发射
#define TIME_SWITCH_SCENE_ANDROID_START     33									// 场景机器人停止发射
	...
	case IDI_SWITCH_SCENE_BEING:
		{
			...
			WORD wRandomTimeEnd=rand()%4+TIME_SWITCH_SCENE_END;
			WORD wRandomTimeStop=rand()%4+TIME_SWITCH_SCENE_ANDROID_STOP;
			WORD wRandomTimeStart=rand()%4+TIME_SWITCH_SCENE_ANDROID_START;
...
bool CAndroidUserItemSink::OnSubExchangeFishScore(void* data, WORD data_size) {
  ...
  if (exchange_fishscore->chair_id == m_pIAndroidUserItem->GetChairID()) {
    ...
    if (exchange_times_ <= 0) {
      m_pIAndroidUserItem->SetGameTimer(IDI_FIRE,(m_pIAndroidUserItem->GetChairID()+m_pIAndroidUserItem->GetTableID())%2+2 );
    } else {
      m_pIAndroidUserItem->SetGameTimer(IDI_EXCHANGE_FISHSCORE, TIME_EXCHANGE_FISHSCORE);
    }
  }
  ...
  if (chair_id == 3) {
	  ...
	  if (fire_mode==3)
	  {
		  for (int i = 0; i < CountArray(kFireAngleL); ++i) {
		   ...
		   {
		    idx = (rand()%CountArray(kFireAngleL));	  
		   }
		  }

		  idx=(idx%CountArray(kFireAngleL));

		  angle = kFireAngleL[idx];

		  last_fire_angle_ = angle;
	  }
	  else{
		  for (int i = 0; i < CountArray(kFireLittleAngleL); ++i) {
			  if (last_fire_angle_ == kFireLittleAngleL[i]) 
			  {
				  idx = i;
				  idx++;
				  break;
			  ...
			  {
				  idx = (rand()%CountArray(kFireLittleAngleL));	  
			  }
		  }

		  idx=(idx%CountArray(kFireLittleAngleL));

		  angle = kFireLittleAngleL[idx];
	...
	//  ----------------------------------------------------
	  ...
	  if (chair_id == 4)
	  {
		  if (fire_mode==3)
		  {
			  for (int i = 0; i < CountArray(kFireAngleFor4Cricle); ++i) {
			   if (last_fire_angle_ == kFireAngleFor4Cricle[i]) 
			   {
			    idx = i;
			    idx++;
			    break;
			   ...
			   {
			   idx = (rand()%CountArray(kFireAngleFor4Cricle));	  
			   }
			  }

			  idx=(idx%CountArray(kFireAngleFor4Cricle));

			  angle = kFireAngleFor4Cricle[idx];
			  
			  last_fire_angle_ = angle;
			  angle += M_PI;
		  }
		  else{
			  for (int i = 0; i < CountArray(kFireAngleFor4LittleCricle); ++i) {
				  if (last_fire_angle_ == kFireAngleFor4LittleCricle[i]) 
				  {
					  idx = i;
					  idx++;
					  break;
				  ...
				  {
					  idx = (rand()%CountArray(kFireAngleFor4LittleCricle));	  
				  }
			  }

			  idx=(idx%CountArray(kFireAngleFor4LittleCricle));

			  angle = kFireAngleFor4LittleCricle[idx];
	  ...
	  else if ( chair_id == 6)
	  {
		  if (fire_mode==3)
		  {
			  for (int i = 0; i < CountArray(kFireAngleFor6Cricle); ++i) {
			   if (last_fire_angle_ == kFireAngleFor6Cricle[i]) 
			   {
			    idx = i;
			    idx++;
			    break;
			   ...
			   {
			    idx = (rand()%CountArray(kFireAngleFor6Cricle));	  
			   }
			  }

			  idx=(idx%CountArray(kFireAngleFor6Cricle));

			  angle = kFireAngleFor6Cricle[idx];

			  last_fire_angle_ = angle;
			  angle += M_PI;
		  }
		  else{
			  for (int i = 0; i < CountArray(kFireAngleFor6LittleCricle); ++i) {
				  if (last_fire_angle_ == kFireAngleFor6LittleCricle[i]) 
				  {
					  idx = i;
					  idx++;
					  break;
				  ...
				  {
					  idx = (rand()%CountArray(kFireAngleFor6LittleCricle));	  
				  }
			  }

			  idx=(idx%CountArray(kFireAngleFor6LittleCricle));

			  angle = kFireAngleFor6LittleCricle[idx];
	  ...
	  else if ( chair_id == 1)
	  {
		  if (fire_mode==3)
		  {
			  for (int i = 0; i < CountArray(kFireAngleFor1Cricle); ++i) {
			   if (last_fire_angle_ == kFireAngleFor1Cricle[i]) 
			   {
			    idx = i;
			    idx++;
			    break;
			   ...
			   {
			    idx = (rand()%CountArray(kFireAngleFor1Cricle));	  
			   }
			  }

			  idx=(idx%CountArray(kFireAngleFor1Cricle));

			  angle = kFireAngleFor1Cricle[idx];

			  last_fire_angle_ = angle;
		  }
		  else{
			  for (int i = 0; i < CountArray(kFireAngleFor1LittleCricle); ++i) {
				  if (last_fire_angle_ == kFireAngleFor1LittleCricle[i]) 
				  {
					  idx = i;
					  idx++;
					  break;
				  ...
				  {
					  idx = (rand()%CountArray(kFireAngleFor1LittleCricle));	  
				  }
			  }

			  idx=(idx%CountArray(kFireAngleFor1LittleCricle));

			  angle = kFireAngleFor1LittleCricle[idx];
...
float CAndroidUserItemSink::GetAngleForFixFire(WORD chair_id) {
	...
	if (chair_id == 3) {
		...
		for (int i = 0; i < CountArray(kFireAngleLFix); ++i) {
			if (last_fire_angle_ == kFireAngleLFix[i]) {
				idx = i;
				break;
			}
		}
		...
		if (random>90)
		{
			idx=(rand()%CountArray(kFireAngleLFix));

		}


		last_fire_angle_ = angle = kFireAngleLFix[idx];
	} else if (chair_id == 0)
	{
	...
	{
		for (int i = 0; i < CountArray(kFireAngleFor1Fix); ++i) {
			if (last_fire_angle_ == kFireAngleFor1Fix[i]) {
				idx = i;
				break;
			}
		}
		...
		if (random>90)
		{
			idx=(rand()%CountArray(kFireAngleFor1Fix));
		}
		last_fire_angle_ = angle = kFireAngleFor1Fix[idx];
	} else if (chair_id == 2)
	{
	...
	{
		for (int i = 0; i < CountArray(kFireAngleFor4Fix); ++i) {
			if (last_fire_angle_ == kFireAngleFor4Fix[i]) {
				idx = i;
				break;
			}
		}
		...
		if (random>90)
		{
			idx=(rand()%CountArray(kFireAngleFor4Fix));
		}
		last_fire_angle_ = angle = kFireAngleFor4Fix[idx];
		angle += M_PI;
	} else if (chair_id == 5)
	{
	...
	{
		for (int i = 0; i < CountArray(kFireAngleFor6Fix); ++i) {
			if (last_fire_angle_ == kFireAngleFor6Fix[i]) {
				idx = i;
				break;
			}
		}
		...
		if (random>90)
		{
			idx=(rand()%CountArray(kFireAngleFor6Fix));
		}
		last_fire_angle_ = angle = kFireAngleFor6Fix[idx];
		angle += M_PI;
	}
