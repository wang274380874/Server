...
//控制状态
#define CONTROL_STATUS_NORMAL   0                 //普通用户
#define CONTROL_STATUS_BLACK    1                 //黑名单用户
#define CONTROL_STATUS_WHITE    2                 //白名单用户

//控制方式
#define CONTROL_TYPE_NULL     0                 //不受控制
#define CONTROL_TYPE_SCORE      1                 //分数控制
#define CONTROL_TYPE_TIME     2                 //时间控制
...
struct tagGameParameter
{
  ...
  //胜率控制
  bool              bAutoControlEnable;         //控制开关
  SCORE             lJoinBlckWinScore;          //累计赢钱
  SCORE             lJoinWhiteLoseScore;        //累计输钱
...
struct tagUserControlInfo
{
  ...
  BYTE              cbControlType;            //控制方式
  BYTE              cbWinRate;              //获胜概率
  DWORD             dwConsumeTime;              //消耗时间
  SCORE             lAddupWinScore;           //累计赢钱
  SCORE             lAddupLoseScore;          //累计输钱
...
//数据接口
interface IGameDataBaseEngineSink :
public IUnknownEx
{
  //配置接口
...
public:
  //获取状态
  virtual BYTE GetUserControlStatus()=NULL;
  //获取信息
  virtual tagUserControlInfo * GetUserControlInfo()=NULL;
  //设置信息
  virtual VOID SetUserControlInfo(const tagUserControlInfo & UserControlInfo)=NULL;
  //更新状态
  virtual VOID UpdateUserControlStatus(SCORE lJoinBlckWinScore,SCORE lJoinWhiteLoseScore)=NULL;
...
//////////////////////////////////////////////////////////////////////////////////
#ifdef _UNICODE
#define VER_IUserControlSink INTERFACE_VERSION(1,1)
static const GUID IID_IUserControlSink= {0x70a893e1,0x68d1,0x4dbb,0x8c,0x37,0xef,0x84,0xf6,0xa1,0xdc,0x95};
#else
#define VER_IUserControlSink INTERFACE_VERSION(1,1)
static const GUID IID_IUserControlSink= {0xaa58fbc4,0x43c0,0x4321,0xb4,0x28,0x7d,0x7e,0x6f,0xfa,0x93,0xd9};
#endif

//用户控制
interface IUserControlSink :
public IUnknownEx
{
  //获取状态
  virtual BYTE GetDynamicControlStatus(WORD wChairID,BYTE & cbCardLevel)=NULL;
};
...
//任务接口
interface IUserTaskManagerSink :
public IUnknownEx
{
  //任务参数
public:
...
//查询接口
interface IDBCorrespondManager :
