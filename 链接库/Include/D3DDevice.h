...
public:
  ...
  //设置采样
  VOID SetEnableMultiSample(bool bEnableMultiSample)
  {
    m_bEnableMultiSample=bEnableMultiSample;
  }
  //设置颜色
  VOID SetBackColor(D3DCOLOR crBackColor)
  {
...
public:
  //回调接口
  ID3DDeviceSink * GetD3DDeviceSink()
  {
    return m_pID3DDeviceSink;
  }
  //设备接口
...
public:
  //获取默认纹理
  IDirect3DTexture9*  GetDefaultTex()
  {
    return m_pDefaultTex;
  };
  //获取默认材质
  D3DMATERIAL9* GetDefaultMaterial()
  {
...
public:
  //获取摄像机
  CD3DCamera* GetD3DCamera()
  {
    return &m_D3DCamera;
  };
  //获取当前观察点
  D3DXMATRIX* GetCurrentView()
  {
    return m_D3DCamera.GetCurrentView();
  };
  //获取默认观察点
  D3DXMATRIX* GetDefaultView()
  {
    return m_D3DCamera.GetDefaultView();
  };

  //效果池信息
public:
  //获取效果池
  CD3DEffectPool * GetD3DEffectPool()
  {
    return &m_D3DEffectPool;
  };

  //获取投影矩阵
public:
  //获取正交
  D3DXMATRIX* GetMatrixOrtho()
  {
    return &m_MatrixOrtho;
  };
  //获取透视
  D3DXMATRIX* GetMatrixPerspective()
  {
