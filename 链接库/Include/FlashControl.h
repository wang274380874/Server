...
protected:
  ...
  LONG              m_lLastFrames;            //帧数变量
  LONG              m_lLastFrameCount;          //帧数变量
...
public:
  ...
  //可视控制
  BOOL ShowControl(WORD wVisible)
  {
    return ::ShowWindow(m_hWnd,wVisible);
  };
...
public:
  ...
  virtual HRESULT STDMETHODCALLTYPE QueryService(REFGUID guidService, REFIID IID,VOID __RPC_FAR *__RPC_FAR *ppvObject)
  {
    return 0;
  }
  virtual HRESULT STDMETHODCALLTYPE GetIDsOfNames(REFIID IID, LPOLESTR __RPC_FAR *rgszNames,UINT cNames, LCID lcid,DISPID __RPC_FAR *rgDispId)
  {
    return E_NOTIMPL;
  }
  virtual HRESULT STDMETHODCALLTYPE Invoke(DISPID dispIdMember,REFIID IID,LCID lcid,WORD wFlags,DISPPARAMS __RPC_FAR *pDispParams,VARIANT __RPC_FAR *pVarResult,EXCEPINFO __RPC_FAR *pExcepInfo,UINT __RPC_FAR *puArgErr)
  {
    return S_OK;
  }
...
public:
  ...
  //可视控制
  VOID ShowControl(WORD wVisible);
};
