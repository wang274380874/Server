...
interface IInsureCounterAction
{
  ...
  //登录事件
  virtual VOID PerformLogonInsure(LPCTSTR pszInsurePass)=NULL;
  //校验昵称
  virtual VOID PerformCheckNickName(LPCTSTR pszNickName)=NULL;
  //转帐事件
...
public:
  ...
  CEditUI *           m_pEditAmountDX;          //银行密码

  //界面控件
public:
  CLabelUI *            m_pLabelTarget;           //目标标签
  CButtonUI *           m_pButtonRefresh;         //刷新按钮
  CButtonUI *           m_pButtonCheck;           //查询按钮
  CButtonUI *           m_pButtonConfirm;         //确认按钮
  CButtonUI *           m_pButtonClear;           //清空按钮
  CButtonUI *           m_pButtonScore[5];          //快捷按钮
...
protected:
  ...
  VOID OnBnCheckedScoreButton(CControlUI* pControlUI);
  //清空按钮
  VOID OnBnClickedClearScore();
  //刷新游戏币
  VOID OnBnClickedRefreshScore();
  //查询昵称
  VOID OnBnClickedCheckNickName();
};
...
//银行设置
class CDlgInsureConfig : public CFGuiWnd
{
  //友元定义
  friend class CDlgInsureOperater;
...
protected:
  ...
  //控件变量
protected:
  CCheckButtonUI *        m_pCheckButtonUI;         //复选控件
...
public:
  ...
  CTabButtonUI *          m_TabButton[4];           //选择按钮
...
public:
  ...
  CDlgInsureConfig        m_DlgInsureConfig;          //银行设置
...
//////////////////////////////////////////////////////////////////////////////////
//银行登录
class CDlgInsureLogon : public CFGuiWnd
{
  //对象指针
protected:
  CDlgInsureMain *        m_pDlgInsureMain;         //窗口指针
...
protected:
  CLabelUI *            m_pLabelRound;            //环绕标签
  CButtonUI *           m_pButtonOK;            //确定按钮
  CButtonUI *           m_pButtonCancel;          //取消按钮
  CEditUI *           m_pEditInsurePass;          //密码控件
...
protected:
  ...
  bool              m_bLogonMission;          //登录任务
  bool              m_bCheckMission;          //校验任务
...
public:
  ...
  tagTansferProofInfo       m_TansferProofInfo;         //凭证内容

  //控件变量
protected:
  CDlgInsureSetup         m_DlgInsureSetup;         //银行设置
  CDlgInsureLogon         m_DlgLogonInsure;         //银行登录
...
protected:
  ...
  //登录事件
  virtual VOID PerformLogonInsure(LPCTSTR pszInsurePass);
  //校验昵称
  virtual VOID PerformCheckNickName(LPCTSTR pszNickName);
  //转帐事件
