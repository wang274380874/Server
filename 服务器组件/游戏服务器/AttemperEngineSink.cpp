			...
			case IDI_DISTRIBUTE_ANDROID:	//分配机器
			{
				...
				if (m_AndroidUserManager.GetAndroidCount() > 0 && m_pGameServiceOption->wServerType != GAME_GENRE_MATCH)
				{
					...
					if (bAllowAndroidAttend == true)
					{
						...
						//add by yang yang   2018年9月21 13:53:23
					...
					if (bAllowAndroidAttend == true)
					{
						...
						//add by yang yang   2018年9月21 13:53:42
...
bool CAttemperEngineSink::OnEventUserItemScore(IServerUserItem * pIServerUserItem, BYTE cbReason)
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("即时写分异常: pIServerUserItem->GetUserID()=%ld"), pIServerUserItem->GetUserID());
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	//通知桌子
...
bool CAttemperEngineSink::OnDBCreateSucess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	OutputDebugString(TEXT("私人房间创建成功，设置私人房间参数 ！\n"));
...
bool CAttemperEngineSink::OnTCPNetworkSubUserStandUp(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("用户起立异常: pIServerUserItem->GetUserID()=%ld"), pIServerUserItem->GetUserID());
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	return true;
}
...
bool CAttemperEngineSink::OnTCPNetworkSubRequestReply(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	TCHAR szOut[256] = TEXT("");
	wsprintf(szOut, TEXT("CAttemperEngineSink::OnTCPNetworkSubRequestReply  取消答复 pRequestReply->dwTableID = %d，pIServerUserItem->GetUserID() = %d,pRequestReply->dwUserID = %d\n"), 
		pRequestReply->dwTableID, 
		pIServerUserItem->GetUserID(),
		pRequestReply->dwUserID);
	OutputDebugString(szOut);
	if (pRequestReply->dwTableID == INVALID_TABLE)
	{
		OutputDebugString(TEXT("请求解散无效的桌子 ！ \n"));
		return false;
	}
	//获取桌子
	...
	//add by yang yang   2018年10月9 14:03:02
...
bool CAttemperEngineSink::SwitchUserItemConnect(IServerUserItem * pIServerUserItem, TCHAR szMachineID[LEN_MACHINE_ID], WORD wTargetIndex, BYTE cbDeviceType, WORD wBehaviorFlags, WORD wPageTableCount)
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("切换连接异常: pIServerUserItem->GetUserID()=%ld"), pIServerUserItem->GetUserID());
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	return true;
}
