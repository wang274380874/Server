...
//私人房间
#define IDI_PERSONAL_BEFORE_BEGAIN_TIME		TIME_TABLE_SINK_RANGE+23		//多长时间未开始游戏解散桌子	
#define IDI_PERSONAL_AFTER_BEGIN_TIME			TIME_TABLE_SINK_RANGE+24		////一局开始多长时间后解散桌子 
		...
		for (WORD i = 0; i < m_wChairCount; i++)
		{
			...
			if (pIServerUserItem != NULL)
			{
				...
				catch (...)
				{
					...
					_sntprintf(szMessage, CountArray(szMessage), TEXT("开始设置异常: pIServerUserItem->GetUserID()=%ld"), pIServerUserItem->GetUserID());
...
bool CTableFrame::DismissGame()
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("解散游戏异常: m_wChairCount=%d"), m_wChairCount);
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	return false;
}
...
bool CTableFrame::ConcludeGame(BYTE cbGameStatus)
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("结束游戏异常: m_wChairCount=%d"), m_wChairCount);
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	return true;
}
...
SCORE CTableFrame::CalculateRevenue(WORD wChairID, SCORE lScore)
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("计算税收异常: m_wChairCount=%d"), m_wChairCount);
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	return 0L;
}
...
bool CTableFrame::OnEventUserOffLine(IServerUserItem * pIServerUserItem)
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("断线事件异常: m_wChairCount=%d"), m_wChairCount);
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	return true;
}
...
bool CTableFrame::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter)
{
	...
	case IDI_OFF_LINE:	//断线事件
	{
		...
		{
			TCHAR szMessage[128] = TEXT("");
			_sntprintf(szMessage, CountArray(szMessage), TEXT("断线事件异常 IDI_OFF_LINE: m_wChairCount=%d"), m_wChairCount);
			CTraceService::TraceString(szMessage, TraceLevel_Normal);
		}
		return true;
	}
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("断线事件定时器异常: m_wChairCount=%d"), m_wChairCount);
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	//错误断言
	ASSERT(FALSE);
...
bool CTableFrame::OnEventSocketFrame(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	case SUB_GF_USER_READY:		//用户准备
	{
		...
		//	m_pIMainServiceFrame->InsertDistribute(pIServerUserItem);
			...
			////用户起立
			//PerformStandUpAction(pIServerUserItem);
			//add by yang yang   2018年9月21 13:56:37
			//return true;
...
bool CTableFrame::PerformStandUpAction(IServerUserItem * pIServerUserItem, bool bInitiative/*=false*/)
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("起立动作异常:CTableFrame::PerformStandUpAction m_wChairCount=%d"), m_wChairCount);
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	return true;
}
...
bool CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword)
{
	...
	{
		TCHAR szMessage[128] = TEXT("");
		_sntprintf(szMessage, CountArray(szMessage), TEXT("坐下动作异常:CTableFrame::PerformSitDownAction m_wChairCount=%d"), m_wChairCount);
		CTraceService::TraceString(szMessage, TraceLevel_Normal);
	}
	return true;
}
...
VOID CTableFrame::SetPersonalTableParameter(tagPersonalTableParameter  PersonalTableParameter, tagPersonalRoomOption PersonalRoomOption)
{
	...
	m_pITableFrameSink->SetPersonalGameRule();
}
...
VOID CTableFrame::SetTimerNotBeginAfterCreate()
{
	...
	{
		OutputDebugString(TEXT("CTableFrame::SetTimerNotBeginAfterCreate()\n"));
		return;
	}
...
{
	TCHAR szInfo[260] = { 0 };
	wsprintf(szInfo, TEXT("ptdt *** GetRecordTableOwner = %d \n"), m_dwRecordTableID);
