...
bool CLockTimeMatch::SendReviveNotify(IServerUserItem * pIServerUserItem, BYTE cbReviveTimesed, bool bUseSafeCard)
{
	if (bUseSafeCard==false)
	{
		...
		//构造消息
		_sntprintf(MatchRevive.szNotifyContent,CountArray(MatchRevive.szNotifyContent),TEXT("您的比赛币低于 %d，将不能继续游戏。 您只需支付 %I64d 金币便可复活继续比赛，您确定要复活吗？"),m_pLockTimeMatch->lMatchCullScore,m_pLockTimeMatch->lReviveFee);
